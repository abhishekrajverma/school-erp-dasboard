...
function StudentPortalContent() {
  ...
  const bookIssues = getStudentBookIssues(studentId)
  const exams = student ? getStudentExams(student.class) : []
  const timetable = student ? getStudentTimetable(student.class) : []
  const notices = getStudentNotices()
  ...
  if (!student) {
    return (
      <PortalLayout>
        <p className="text-muted-foreground">Student account not found.</p>
      </PortalLayout>
    )
  }
        ...
        {activeTab === 'overview' && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <StatCard title="Fee status" value={student.feeStatus} icon={CreditCard} />
              <StatCard title="Pending fees" value={formatCurrency(totalPending)} icon={CreditCard} />
              <StatCard title="Upcoming exams" value={exams.filter((e) => e.status === 'scheduled').length} icon={FileText} />
            </div>
            <Card>
              <CardHeader>
              ...
              <CardContent className="space-y-2">
                {notices.map((n) => (
                  <div key={n.id} className="flex justify-between rounded-lg border border-border p-3 text-sm">
                    <span>{n.title}</span>
                    <Badge variant="secondary">{formatDate(n.date)}</Badge>
                  </div>
                ))}
              </CardContent>
            </Card>
